...
// В результате необходимо получить строку, 
// по следующему примеру: «Дорогой Илон Маск, от всего сердца 
// поздравляю тебя с днем рождения, желаю космического терпения, 
...
$epithets = ['бесконечного', 'крепкого', 'большого'];
$name = readline("Введите имя именинника: ");
$resArr = [];

for ($i = 0; $i < 3; $i++) {
  $resArr[] = $epithets[array_rand($epithets)] . ' '. $wishes[array_rand($wishes)];
}

$finalStr = "Дорогой $name, от всего сердца поздравляю тебя с днем рождения, желаю " . implode(', ', $resArr) . '!';
echo $finalStr;
